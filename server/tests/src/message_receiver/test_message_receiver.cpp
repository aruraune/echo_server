...
#include "echo_config/server_config.hpp"
...
auto receive_message<messages::login_response>(client_channel& channel,
  ...
  auto actual_status_code = echo_status::login_status{};

  switch (status_code) {
    case static_cast<std::uint16_t>(echo_status::login_status::OK):
    case static_cast<std::uint16_t>(echo_status::login_status::FAILED):
      actual_status_code = static_cast<echo_status::login_status>(status_code);
      break;
