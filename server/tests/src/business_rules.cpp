...
BOOST_AUTO_TEST_CASE(login_with_password) {
  ...
  echo_server::spawn_server(
      ...
      {
          .port = test_tcp_port,
          .authenticator = echo_server::auth::test_client_authenticator::create(),
      });
...
BOOST_AUTO_TEST_CASE(login_failure) {
  ...
  boost::asio::co_spawn(
      ...
      [&]() -> boost::asio::awaitable<void> {
        ...
        BOOST_CHECK(login_response.status_code ==
                    echo_status::login_status::FAILED);
        BOOST_CHECK(login_response.header.sequence == login_request_sequence);
...
BOOST_AUTO_TEST_CASE(long_message_success) {
  spdlog::set_level(spdlog::level::debug);
  ...
  boost::asio::co_spawn(
      ...
      [&]() -> boost::asio::awaitable<void> {
        ...
        const auto echo_message = std::string(65529, 'M');
...
BOOST_AUTO_TEST_CASE(short_message_success) {
  spdlog::set_level(spdlog::level::debug);
  ...
  boost::asio::co_spawn(
      ...
      [&]() -> boost::asio::awaitable<void> {
        ...
        const auto echo_message = std::string(1, 'M');
...
BOOST_AUTO_TEST_CASE(empty_message_success) {
  spdlog::set_level(spdlog::level::debug);
  ...
  boost::asio::co_spawn(
      ...
      [&]() -> boost::asio::awaitable<void> {
        ...
        const auto echo_message = std::string{};
